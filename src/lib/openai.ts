...
export async function generatePromptResponse(prompt: string, model: 'gpt-4' | 'gpt-3.5-turbo'): Promise<PromptResponse> {
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      messages: [
        {
          role: "system",
          content: "You are a professional blog writer. Write engaging, well-structured blog posts with proper formatting using HTML. Include headings (h1, h2, h3), paragraphs, and proper emphasis where needed."
        },
        { 
          role: "user", 
          content: prompt 
